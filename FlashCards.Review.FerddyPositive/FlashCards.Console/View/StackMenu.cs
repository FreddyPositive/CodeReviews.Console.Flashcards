...
internal class StackMenu
{
    ...
    private void CreateStack()
    {
        ...
        bool IsStackAlreadyExist = stackController.CheckStackNameAvailable(stackName);

        while (string.IsNullOrEmpty(stackName) || stackName.Length < 2 || !Regex.IsMatch(stackName, @"^[a-zA-Z #\-/]+$") || IsStackAlreadyExist)
        {
            if (stackName == "0")
            {
                ManageStacks();
            ...
            if (IsStackAlreadyExist = true) Console.WriteLine("Stack Name Already Available!!");

            Console.WriteLine();
            Console.WriteLine("Please Enter a Proper Stack Name");
            stackName = Console.ReadLine();

            IsStackAlreadyExist = stackController.CheckStackNameAvailable(stackName);
        }
    ...
    private void RemoveStack()
    {
        ...
        Console.WriteLine("Please note that by deleting a stack the related flash cards and the study sessions will also get deleted!!!");
        Console.WriteLine();
        Console.WriteLine("Please Enter a valid Stack Id, To Return to Stack Menu Enter 0");
        int StackId = ValidateStackId();
